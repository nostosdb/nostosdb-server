...
- `release-manifest.json`, `scripts/stage_npm_candidate.py`, and the `npm/` package tree define the unpublished `@nostdb/server` launcher plus six exact native packages. `scripts/verify_local_npm.py` proves an offline isolated global install exposes both `nostd` and `nostdb` through the exact matching CLI package.
